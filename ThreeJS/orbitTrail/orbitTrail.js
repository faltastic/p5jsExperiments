...
function init() {
   ...
   var effectController = {
    ...
    for(var n=0;n<TotalBoxes; n++){

         boxes[n] = new THREE.Mesh(new THREE.BoxGeometry(10, 10, 10),
            hydraStroke);

         boxes[n].position.x = 200*Math.random();
         boxes[n].position.y = 200*Math.random();
         boxes[n].position.z = 200*Math.random();
        boxes[n].scale.set(0.3,0.3,0.3);
       scene.add(boxes[n]);
    }

    //scene.add(boxes[0]);//scene.add(hydra[0]);
...
function render() {
    ...
    if (camPosIndex > 4) {
       var nextStop = spline.getPoint(( camPosIndex + (70 * TotalBoxes+1)) / 10000);
        boxes[0].position.x = nextStop.x;
        boxes[0].position.y = nextStop.y;
        boxes[0].position.z = nextStop.z;
    }
    else {

         var nextStops = [];
        for (var n = 0; n < TotalBoxes; n++) {
            nextStops[n] = spline.getPoint((camPosIndex + 70 * n) / 10000);
            boxes[n].position.x = nextStops[n].x;
            boxes[n].position.y = nextStops[n].y;
            boxes[n].position.z = nextStops[n].z;
        }
